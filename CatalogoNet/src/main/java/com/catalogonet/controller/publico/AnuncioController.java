...
public class AnuncioController {
	...
	@RequestMapping(value = { "{titulo}-{categoria}-{bairro}-{cidade}-{estadoId}-{id}",
			"{titulo}-{categoria}-{cidade}-{estadoId}-{id}" })
	public String mostraAnuncio(@PathVariable("titulo") String titulo, @PathVariable("categoria") String categoria,
			@PathVariable(value = "bairro") String bairro, @PathVariable("cidade") String cidade,
			@PathVariable("estadoId") String estadoId, @PathVariable("id") String id, ModelMap map) {
		...
		if (anuncio != null) {
			...
			desc.append("Venha conhecer ").append(anuncio.getTitulo())
					...
					.append(anuncio.getCategoria().getNome()).append(", localizado em ")
					.append(anuncio.getBairro().getNome()).append(",").append(anuncio.getCidade().getNome()).append(",")
					.append(anuncio.getEstado().getId());

			tit.append(anuncio.getTitulo()).append(" - ").append(anuncio.getBairro().getNome()).append(" - ")
